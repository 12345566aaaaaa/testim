...
export default function GazetarDashboard() {
  ...
  const handleFileUpload = async (e, type, isEditing = false) => {
    try {
      ...
      if (isEditing) setEditingArticle({ ...editingArticle, image_url: data.publicUrl });
      else setNewArticle({ ...newArticle, [type === 'images' ? 'image_url' : 'video_url']: data.publicUrl });
  ...
  const handleSubmitArticle = async (e) => {
    e.preventDefault();
    const { data: { user } } = await supabase.auth.getUser();
    const { error } = await supabase.from('articles').insert([{ ...newArticle, author_id: user.id, status: 'pending' }]);
    if (!error) {
      ...
      <div className="flex-1 p-8 overflow-y-auto">
        ...
        {activeTab === 'create' && (
          <div className="max-w-3xl mx-auto">
            ...
            <form onSubmit={handleSubmitArticle} className="space-y-5 bg-white dark:bg-slate-900 p-8 rounded-[30px] shadow-sm border border-gray-50 dark:border-slate-800">
              <div>
                <label className="block text-[10px] font-black uppercase text-gray-400 mb-1.5 tracking-widest">Titulli</label>
                <input required type="text" value={newArticle.title} onChange={(e) => setNewArticle({...newArticle, title: e.target.value})} className="w-full p-3.5 bg-gray-50 dark:bg-slate-800 rounded-2xl outline-none font-bold text-md dark:text-slate-200" placeholder="Titulli..." />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <input type="file" accept="image/*" onChange={(e) => handleFileUpload(e, 'images')} className="text-xs text-gray-500" />
                <input type="file" accept="video/*" onChange={(e) => handleFileUpload(e, 'videos')} className="text-xs text-gray-500" />
              </div>
              <textarea required rows="7" value={newArticle.content} onChange={(e) => setNewArticle({...newArticle, content: e.target.value})} className="w-full p-3.5 bg-gray-50 dark:bg-slate-800 rounded-2xl outline-none text-md dark:text-slate-300" placeholder="Shkruaj tekstin..."></textarea>
              <button type="submit" disabled={uploading} className="w-full py-4 bg-[#f59e0b] text-white rounded-2xl font-black shadow-lg disabled:opacity-50 text-sm uppercase leading-none">
              ...
              {paginatedData.map(art => (
                <div key={art.id} className="bg-white dark:bg-slate-900 p-4 rounded-2xl border border-gray-100 dark:border-slate-800 shadow-sm flex flex-col group hover:shadow-md transition-all">
                  <div className="flex justify-between items-center mb-3">
                    ...
                    <button onClick={() => setEditingArticle(art)} className="p-1.5 bg-blue-50 text-blue-600 rounded-lg text-[10px] font-bold opacity-0 group-hover:opacity-100 transition-all">Edito</button>
                  </div>
                  <h3 className="font-bold text-[#1a5f7a] dark:text-[#38bdf8] text-sm mb-1.5 line-clamp-2 h-10">{art.title}</h3>
                  {art.image_url && <div className="mt-auto relative w-full h-28 rounded-xl overflow-hidden mb-1.5 shadow-sm border dark:border-slate-800"><Image src={art.image_url} alt="Media" fill className="object-cover" /></div>}
